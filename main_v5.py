...
class WaterHeaterRecommendationSystem:
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            self.sites_info = DataLoader.load_file(DATA_FILES['sites_info'])
    ...
    def _set_fallback_data(self) -> None:
        ...
        self.sites_info = "Technical information not available."
...
class RAGManager:
    """Manages RAG system interactions"""
    
    def __init__(self, rag_system):
        self.rag_system = rag_system
    
    async def generate_rag_response(self, message: str, state: ConversationState) -> AsyncGenerator[str, None]:
        """Generate RAG response with proper error handling and streaming"""
        try:
            if not self.rag_system:
                yield json.dumps({'content': 'Technical information system is not available right now.', 'done': True})
                return
            
            full_response = ""
            
            # Process the async stream
            async for chunk in self.rag_system.get_full_file_response(message):
                if chunk:
                    full_response += chunk
                    yield f"data: {json.dumps({'content': chunk, 'done': False})}\n\n"
            ...
            error_response = "I'm having trouble accessing the technical information right now."
...
def get_rag_manager() -> RAGManager:
    ...
    return RAGManager(app_state.rag_system)
...
async def send_message(
    ...
    if classification.type == "question" and classification.confidence > 0.6:        
        return StreamingResponse(
            rag_manager.generate_rag_response(chat_message.message, state),
