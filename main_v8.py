...
class WaterHeaterSystemComponents:
    ...
    def _create_prompts(self):
        ...
        system_prompt_text = f"""
            You are a friendly, knowledgeable water heater consultant who genuinely cares about helping homeowners find the perfect water heater solution. You have years of experience helping families make smart decisions about their home's hot water needs.
            ...
            4. Budget considerations and priorities (cost savings, efficiency, reliability)
            5. Space and installation constraints
            6. Any past experiences or preferences

            Remember:
            - Ask only one question at a time
            - You're having a conversation, not conducting an interview
            - Let the discussion flow naturally based on their responses
            - Show empathy for their situation and concerns
            - Be genuinely helpful and supportive throughout
            - Don't repeat the same question - adapt based on their responses

            Your goal is to make them feel heard, understood, and confident in their decision while gathering the information needed to provide the best possible recommendation for their specific situation.
        """
...
def classify_user_input(state: GraphState) -> GraphState:
    """Node: Classify user input as 'continue_conversation' or 'need_rag_help'."""
    ...
    try:
        ...
        # Build conversation history (up to last 4 messages for context)
...
def check_trigger_conditions(state: GraphState) -> GraphState:
    ...
    force_keywords = [
        'recommend', 'give recommendations', 'show options', 'compare', 'best option', 'what do you recommend', 'suggest', 'help me choose', 'pick one', 'decision', 'choose'
    ]
...
def log_state(state: GraphState, node_name: str, direction: str = "IN"):
    ...
    # print(f"\n==== {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} === {direction} {node_name.upper()} ====")
    # print(json.dumps(state, indent=2, default=str))
    # print(f"==== END {direction} {node_name.upper()} ====\n")
