...
async def classify_user_input(user_message: str, conversation_context: list) -> dict:
    """
    Classify user input as either a question (needs RAG) or an answer (continue conversation).
    Returns: {"type": "question" | "answer", "confidence": float, "reasoning": str}
    """
    ...
    Analyze the user's input and classify it as either:
    1. "question" - User is asking a new question that requires information lookup
    2. "answer" - User is responding to a previous question or providing information
    
    Context - Last AI message: "{last_ai_message}"
    User input: "{user_message}"
    
    Consider:
    - Question indicators: "what", "how", "why", "when", "where", "can you", "do you know", "explain", "tell me"
    - Answer indicators: Providing specific information, confirming/denying, giving preferences/details
    - If the user is clearly responding to what the AI asked, it's likely an "answer"
    - If the user is asking for new information, it's likely a "question"
    
    Respond in JSON format: {{"type": "question" or "answer", "confidence": 0.0-1.0, "reasoning": "brief explanation"}}
    """
    
    try:
        # Use OpenAI (or LangChain) to classify the input as question or answer
        llm = ChatOpenAI(
        ...
        if match:
            try:
                ...
                if "type" in result and "confidence" in result and "reasoning" in result:
                    return result
            except json.JSONDecodeError:
                logger.warning(f"Failed to parse JSON from response: {response_content}")

        # Fallback: treat as answer if parsing fails
        return {
            "type": "answer",
            "confidence": 0.5,
        ...
        # Default to answer to continue conversation flow
        return {
            "type": "answer",
            "confidence": 0.5,
...
def save_conversation_to_file(messages: List[Dict[str, str]], filename: str = "conversation_log.txt"):
    """Save conversation to a file in the desired format."""
    with open(filename, "a", encoding="utf-8") as f:
        for msg in messages[-2:]:  # Only save the last exchange (user + assistant)
            if msg["role"] == "user":
...
async def send_message(chat_message: ChatMessage):
    ...
    if chat_message.message.lower() in ['restart', 'start over', 'reset']:
        ...
        save_conversation_to_file(messages)
        return {
            "message": messages[-1]['content'],
            "is_complete": False
        }
    
    # Classify user input using the agent
    classification = await classify_user_input(chat_message.message, state.messages)
    logger.info(f"Input classification: {classification}")
    
    # Handle based on classification
    if classification["type"] == "question" and classification["confidence"] > 0.6:
        # User is asking a question - use RAG system
        logger.info(f"Classified as question, using RAG for: {chat_message.message}")
        
        async def generate_rag_then_continue():
            """Generate RAG response then return to normal conversation flow"""
            # First, stream the RAG response
            rag_response = ""
            async for chunk in generate_rag_response(chat_message.message, state):
                yield chunk
                # Capture the full RAG response
                if '"done": true' in chunk:
                    try:
                        data = json.loads(chunk.split("data: ")[1])
                        if 'full_response' in data:
                            rag_response = data['full_response']
                    except:
                        pass
            
            save_conversation_to_file(state.messages)
            # Add the RAG response to conversation history
            if rag_response:
                state.messages.append({"role": "user", "content": chat_message.message})
                state.messages.append({"role": "assistant", "content": rag_response})
                
                # Add a transition message to continue the consultation
                transition_msg = "Now, let's continue with your water heater consultation. Is there anything else you'd like to know, or shall we proceed with the recommendations?"
                state.messages.append({"role": "assistant", "content": transition_msg})
                
                # Yield the transition message
                final_data = {
                    'content': f"\n\n{transition_msg}",
                    'done': True,
                    ...
                    'is_complete': False,
                    'using_rag': True,
                    'transitioned_back': True
                }
                yield f"data: {json.dumps(final_data)}\n\n"
        
        return StreamingResponse(
            generate_rag_then_continue(),
            media_type="text/plain",
            headers={"Cache-Control": "no-cache", "Connection": "keep-alive"}
        )
    
    else:
        # User is providing an answer or continuing conversation - use normal recommendation flow
        logger.info(f"Classified as answer, using recommendation system for: {chat_message.message}")
