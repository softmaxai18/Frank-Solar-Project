...
async def classify_user_input(state: GraphState) -> GraphState:
    ...
    try:
        ...
        classification_prompt = f"""
            ...
            {{"type": "question" or "answer","confidence": float (range: 0.0 to 1.0),"reasoning": "Brief justification of your classification decision"}}
        """
...
def check_trigger_conditions(state: GraphState) -> GraphState:
    ...
    if turn_count >= 5 and has_sufficient_info(messages):
    ...
    # Force recommendation after 8 turns regardless
    if turn_count >= 8:
        logger.info("Forcing recommendation after 8 turns")
        return {**state, "decision": "recommend_types"}
...
async def generate_recommendation(state: GraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        system_prompt = f"""
            ...
            THESE ARE WATER HEATER SYSTEMS TYPE :
            {json.dumps(system_components.summary_table, indent=2) if system_components.summary_table else "No summary data available"}
            Use this to give 6 rank base recommendation
...
def has_sufficient_info(messages: List[BaseMessage]) -> bool:
    ...
    essential_info = {
        ...
        'priorities': ['cost', 'save', 'efficient', 'reliable', 'environment', 'important', 'matter', 'budget', 'cheap', 'expensive', 'upfront', 'long term', 'efficiency','balance']
    }
...
def log_state(state: GraphState, node_name: str, direction: str = "IN"):
    ...
    logger.info(f"{color}================ {direction} {node_name.upper()} ======={reset}")
    ...
    for field, value in essential_fields.items():
        logger.info(f"{Fore.YELLOW}{field:>20}{reset}: {str(value).strip()}")
    
    logger.info(f"{color}================ END {direction} ================{reset}\n")
